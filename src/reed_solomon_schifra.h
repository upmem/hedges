...
union b8
{
	...
	{
		if (vec.size() != 8)
			throw("bad conversion of VecUchar to b8");
		for (int i = 0; i < 8; i++)
			b[i] = vec[i];
	}
...
{ // see Schifra's example08
	...
	VecUchar encode(VecUchar &mess)
	{
		...
		for (int i = 0; i < code_length; i++)
			codeword[i] = static_cast<Uchar>(block.data[i]);
		return codeword;
	}
	b8 encode(b8 mess)
	{
					...
					Int &err_number, bool &recoverable)
	{
		...
		for (int i = 0; i < code_length; i++)
			message[i] = static_cast<Uchar>(block.data[i]);
					...
					Int &err_number, bool &recoverable)
	{
		...
		for (int i = 0; i < esize; i++)
			erasures_long[i] = std::size_t(erasures[i]);
		block.reset(); // not needed?
		for (int i = 0; i < code_length; i++)
			block.data[i] =
				static_cast<galois::field_symbol>(codeword[i]);
			  ...
			  Int &err_number, bool &recoverable)
	{
		...
		b8 message;
		for (int i = 0; i < code_length; i++)
			message.b[i] = static_cast<Uchar>(block.data[i]);
		errs_detected = Int(block.errors_detected);
		errs_corrected = Int(block.errors_corrected);
		recoverable = !block.unrecoverable;
		return message;
